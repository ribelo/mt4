            ...
            for (int i = 0; i < total_history; i++) {
                if (OrderSelect(i, SELECT_BY_POS, MODE_HISTORY)) {
                    if (OrderProfit() + OrderCommission() != 0.0) {
                        ...
                        ArrayResize(balance_array, size);
                        balance_array[size - 1] = curr_balance;
                    }
                }
            }
        }
...
double RiskReward() {
    ...
    static double pips_profit = 0.0;
    static double pips_loss = 0.0;
    static int trades_win = 0.0;
    static int trades_loss = 0.0;
...
double Profitability() {
    ...
    static double trades_win = 0.0;
    static double trades_loss = 0.0;
    int local_multiplier, local_digits;
    if (total_history != OrdersHistoryTotal()) {
        total_history = OrdersHistoryTotal();
        for (int i = 0; i < total_history; i++) {
...
double DynamicDeltaLot(string symbol, double stop_pip, double max_dd,
                       double max_risk, double& balance_array[]) {
    static int total_history;
